...
export const DraftGenerationResultSchema = z.object({
  ...
  subtitle: z.string().max(200).optional().describe('副标题/字幕文案'),
  description: z.string().max(2200).describe('Video description for TikTok (max 2200 chars)'),
  topics: z.array(z.string()).max(10).describe('Legacy topics field, aligned with hashtags'),
  hashtags: z.array(z.string()).max(10).optional().describe('Hashtags generated by copy engine v2'),
  blueWords: z.array(z.string()).optional().describe('Blue words for Xiaohongshu interaction'),
  commentGuide: z.string().optional().describe('Combined comment guide text'),
  commentGuides: z.array(z.string()).optional().describe('Three comment guide suggestions'),
  copyHistoryId: z.string().optional().describe('Copy history id from copy engine'),
