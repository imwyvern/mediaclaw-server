...
const testingHarness = vi.hoisted(() => {
  ...
  const mongoEnumMocks: Record<string, Record<string, string>> = {
    BillingMode: createEnum(['PREPAID', 'POSTPAID']),
    ...
    McUserType: createEnum(['ENTERPRISE', 'PERSONAL']),
    NotificationChannel: createEnum(['EMAIL', 'WEBHOOK', 'IN_APP']),
    NotificationEvent: createEnum(['TASK_COMPLETED', 'PAYMENT_SUCCEEDED', 'SYSTEM_ALERT']),
    OrgStatus: createEnum(['ACTIVE', 'INACTIVE', 'SUSPENDED']),
    OrgType: createEnum(['AGENCY', 'BRAND', 'INTERNAL']),
    ...
    SubscriptionPlan: createEnum(['FREE', 'PRO', 'ENTERPRISE']),
    SubscriptionStatus: createEnum(['ACTIVE', 'CANCELLED', 'EXPIRED']),
    UserRole: createEnum(['ADMIN', 'MEMBER', 'OWNER']),
    ...
    VideoTaskType: createEnum(['REMIX', 'LOCALIZATION', 'RECAP', 'PROMOTION']),
