...
const testingHarness = vi.hoisted(() => {
  ...
  const mongoEnumMocks: Record<string, Record<string, string>> = {
    ...
    DeliveryChannel: createEnum(['FEISHU', 'WECOM', 'WECHAT', 'DINGTALK', 'TELEGRAM']),
    ...
    EnterpriseSsoProtocol: createEnum(['OIDC', 'SAML']),
    EnterpriseSsoProviderType: createEnum(['WECOM', 'DINGTALK', 'FEISHU', 'OIDC', 'SAML']),
    InvoiceStatus: createEnum(['DRAFT', 'ISSUED', 'PAID', 'OVERDUE', 'VOID']),
    LayerBillingModel: createEnum(['FREE', 'QUOTA', 'SUBSCRIPTION', 'USAGE', 'POSTPAID']),
    ...
    RefundRequestStatus: createEnum(['PENDING', 'APPROVED', 'REJECTED', 'PROCESSING', 'REFUNDED', 'FAILED']),
    RefundExecutionMode: createEnum(['MANUAL', 'GATEWAY']),
    ...
    SkillMarketplaceEntryStatus: createEnum(['DRAFT', 'PUBLISHED', 'ARCHIVED']),
    SkillMarketplaceVisibility: createEnum(['PRIVATE', 'ORGANIZATION', 'PUBLIC']),
